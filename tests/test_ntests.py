...
def test_all_modules_have_tests():
    ...
    '''
    f = filter(lambda s: inspect.ismodule(getattr(pyscripts, s)), pyscripts.__all__)
...
def test_all_members_have_tests():
    '''
    Test that all the direct members in "pyscripts" have at least one test. The
    ...
    # Get all the members of "pyscripts"
    fltr = lambda s: not s.startswith('_') and not inspect.ismodule(getattr(pyscripts, s))
    members = tuple(filter(fltr, pyscripts.__all__))
